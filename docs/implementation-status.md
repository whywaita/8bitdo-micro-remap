...
| Vite static development and build                                                       | vite.config.ts, index.html; development browser smoke test with zero page/console errors                          | Passed locally                                                            |
...
| Static assets at the Pages subpath, reload and 404 behavior                             | Pages-subpath browser test and production artifact checks                                                         | Passed locally                                                            |
| Production CSP excludes inline/eval allowances; dev refresh is usable                   | Build-only CSP meta injection in vite.config.ts; production E2E meta-policy assertion; dev console checked        | Passed                                                                    |
...
| CI actionlint job and SHA-pinned actions                                                | .github/workflows/ci.yml; actionlint and pinact run -u                                                            | Passed locally; previous PR CI passed, Pages update CI checked separately |
| Production static build                                                                 | pnpm build                                                                                                        | Passed; no deployment performed                                           |
| Limitations, platforms, recovery, provenance, verification levels                       | README.md and hardware-verification.md                                                                            | Documented                                                                |
| Actual HTTPS origin and headers                                                         | Requires Pages source set to GitHub Actions and main-branch deployment                                            | Pending                                                                   |
...
- Unit/component/architecture suite: 290 tests.
...
- Production artifact checks: 2 tests.
- TypeScript, ESLint, formatting, actionlint, and static production build: passed locally.

These results prove behavior against protocol vectors, browser API doubles, a scripted transport, and local static hosting. They do not prove firmware interoperability or deployment availability.

## Remaining external evidence

1. Set repository Pages source to GitHub Actions, merge the reviewed implementation, and verify the deployed HTTPS site.
...
3. GitHub remote is configured and PR #1 is open. Deployment occurs only on main-branch pushes after CI passes.
...
- Static Pages hosting replaces the Worker. CSP is embedded only in production HTML; referrer policy is a meta tag. Response-header controls and frame-ancestors enforcement are not claimed.
